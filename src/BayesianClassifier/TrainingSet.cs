...
public sealed class TrainingSet : ITrainingSet
{
    ...
    /// The data sets
    /// </summary>
    private readonly ConcurrentDictionary<IClass, IDataSet> _dataSets = new();
    ...
    public IDataSet this[IClass @class]
    {
        get
        {
            if (_dataSets.TryGetValue(@class, out var set)) return set;
            throw new ArgumentException("No data set was registered for the given class", nameof(@class));
        }
    }
    ...
    {
        if (ReferenceEquals(dataSet, null)) throw new ArgumentNullException(nameof(dataSet));
    ...
    {
        if (ReferenceEquals(dataSets, null)) throw new ArgumentNullException(nameof(dataSets));
    ...
    {
        return _dataSets.Select(static dataSet => dataSet.Value).GetEnumerator();
    }
