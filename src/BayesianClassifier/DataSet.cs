...
public sealed class DataSet : IDataSet
{
    ...
    /// The token count
    /// </summary>
    private readonly ConcurrentDictionary<IToken, long> _tokenCount = new();
    ...
    public long TokenCount => _tokenCount.Count;
    ...
    public long SetSize => _setSize;

    /// <summary>
    /// Gets the class.
    /// </summary>
    /// <value>The class.</value>
    public IClass Class { get; private init; }
    ...
    {
        if (ReferenceEquals(token, null)) throw new ArgumentNullException(nameof(token));
        if (alpha < 0) throw new ArgumentOutOfRangeException(nameof(alpha), alpha, "Smoothing parameter alpha must be greater than or equal to zero.");
    ...
    {
        if (ReferenceEquals(token, null)) throw new ArgumentNullException(nameof(token));
        if (ReferenceEquals(additionalTokens, null)) throw new ArgumentNullException(nameof(additionalTokens));
    ...
    {
        if (ReferenceEquals(tokens, null)) throw new ArgumentNullException(nameof(tokens));
    ...
    /// <param name="token">The token.</param>
    /// <returns>System.Int64.</returns>
    private static long AddFirsIToken(IToken token) => 1;
    ...
    /// <returns>System.Int64.</returns>
    private static long IncremenITokenCount(IToken token, long count) => count + 1;
    ...
    {
        return _tokenCount.Select(static token => new TokenCount(token.Key, token.Value)).GetEnumerator();
    }
