...
internal sealed class EmptyDataSet : IDataSet
{
    ...
    public TokenInformation<IToken> this[IToken token, double alpha = 0D] => new(token, 0L, 0D);
    ...
    public long GetCount(IToken token) => 0L;
    ...
    public double GetPercentage(IToken token, double alpha = 0) => 0D;
