...
public abstract class ClassBase : IClass
{
    ...
    {
        if (probability < 0) throw new ArgumentOutOfRangeException(nameof(probability), "Class base probability must be greater than or equal to zero.");
        if (probability > 1) throw new ArgumentOutOfRangeException(nameof(probability), "Class base probability must be less than or equal to one.");
